...
- PHP Version >= 8.0
- Extension `GD` enabled, for image processing
- Support for `Argon2id` hashing, either `libsodium`, `libargon2` or `OpenSSL` (for PHP >= 8.4.0) extension enabled, for password hashing
...
Users are stored under `Login/Accounts/`. Each directory represents a user with its name representing the username. Usernames are therefore unique identifiers. Initially, only three files: `password.txt`, `role.txt` and `pfp.txt` are present and storing the *hashed password*, *JSON-encoded roles* and *profile picture*.
...
Transition to SQLite3 is planned.
