...
<body class="container pt-4">
    ...
    if ($current->accessstatus)
    {
        ...
        <div class="mb-5">
        <h5 class="mt-3">Change profile picture</h5><form method="post" enctype="multipart/form-data"><div class="input-group"><input type="file" class="form-control" name="userprofilepicture" accept="image/*" required><button class="btn btn-primary" type="submit" name="uploadpicsubmit"><i class="bi bi-upload"></i> Upload</button></div></form>
        </div><div class="mb-5">
        <h5 class="mt-3">Change birthday</h5><form method="post" enctype="multipart/form-data"><div class="input-group"><input type="date" class="form-control" name="birthday" placeholder="Date" value="' . date("Y-m-d", $current->getBirthday()) . '"><button class="btn btn-primary" name="changebdaysubmit">Submit</button></div></form>
        </div>';
        }
        echo '<hr><form method="post" enctype="multipart/form-data" id="logoutcurrent"><button type="button" style="width:100%;" class="btn btn-danger btn-lg" onclick="confirmModal(\'Log out?\',\'Are you sure to log out from current device?\',\'$(\\\'#logoutcurrent\\\').submit()\',\'Cancel\',\'Log out\')"><i class="bi bi-box-arrow-right"></i> Log out</button><input type="hidden" name="logout"></form>';
    }
    else
    {
