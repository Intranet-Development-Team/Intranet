...
<body class="container pt-4">
    ...
    if ($current->accessstatus)
    {
        ...
        else if (isset($_GET["changeuserrole"]))
        {
            ...
            echo '<form method="post" style="margin-top:1em;" onsubmit="preventMisclick($(\'#changeuserrole_submit\'))"><select name="changeuserrole_targetuser" class="form-select">' . $userlistoptions . '</select><div class="card p-3 mt-2"><h6>Roles (leave all check boxes empty to clear all roles): </h6>' . $rolelistdisplay . '</div><small class="mt-2 text-muted" style="display:block;">Roles not defined in ROLE_LIST cannot be modified here.</small><button type="submit" class="btn btn-primary mt-2" name="changeuserrole_submit" id="changeuserrole_submit">Submit</button></form>';
        }
        else if (isset($_GET["adduser"]))
        {
            echo '<form method="post" style="margin-top:1em;" onsubmit="preventMisclick($(\'#adduser_submit\'))"><input type="text" class="form-control" placeholder="Username" name="adduser_username" required><input type="password" class="form-control mt-2" placeholder="Password" name="adduser_password" required><button type="submit" class="btn btn-primary mt-2" name="adduser_submit" id="adduser_submit">Submit</button></form>';
        }
        else if (isset($_GET["servicestatus"]))
        {
            if (is_file("maintaining.txt"))
            {
                echo '<h1 class="text-center mt-5 mb-5"><i class="bi bi-pause-fill"></i> Service has been suspended</h1><form method="post" onsubmit="preventMisclick($(\'#updatemsg_submit\'))"><input type="text" name="updatemsg_msg" class="form-control form-control-lg mt-5 mb-5" placeholder="Message" value="' . htmlspecialchars(fileread("maintaining.txt")) . '"><button type="submit" name="updatemsg_submit" id="updatemsg_submit" class="btn btn-primary btn-lg w-100 mb-3"><i class="bi bi-arrow-up-circle-fill"></i> Update message</button></form><form method="post" onsubmit="preventMisclick($(\'#resumeservice\'))"><button type="submit" name="resumeservice" id="resumeservice" class="btn btn-success btn-lg w-100"><i class="bi bi-play-fill"></i> Resume service</button></form>';
            }
            else
            {
                echo '<form method="post" onsubmit="preventMisclick($(\'#suspendservice\'))"><input type="text" name="suspendservice_msg" class="form-control form-control-lg mt-5 mb-5" placeholder="Message" value="We will resume our service shortly."><button type="submit" name="suspendservice" class="btn btn-danger btn-lg w-100" id="suspendservice"><i class="bi bi-pause-fill"></i> Suspend service</button></form>';
            }
        }
        else
        {
            $feedbackcontent = fileread("feedbacks.txt");
            echo '<div class="card card-body" style="max-height:50em;overflow:auto;">' . (empty($feedbackcontent) ? "<h6 style=\"text-align:center;margin-bottom:0em;\">No feedbacks</h6>" : $feedbackcontent) . '</div><form method="post" id="feedbacksdeleteform" onsubmit="preventMisclick($(\'#feedbacksdeleteformsubmitbtn\'))"><button type="button" onclick="confirmModal(\'Confirm clearing feedbacks\',\'Are you sure to clear the feedbacks? This action cannot be undone.\',\'$(&quot;#feedbacksdeleteform&quot;).submit();\');" class="btn btn-lg w-100 mt-4 btn-danger" id="feedbacksdeleteformsubmitbtn"><i class="bi bi-trash3-fill"></i> Clear Feedbacks</button><input type="hidden" name="feedbacksdelete"><form>';
        }
    }
    else
    {
